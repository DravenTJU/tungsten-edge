...
final class PanelCoordinator: NSObject {
    ...
    private func makeDockPanel(on screen: NSScreen, screenID: ScreenID) -> (NSPanel, ManualPanelHost) {
        ...
        hosting.wantsLayer = true
        hosting.layer?.backgroundColor = NSColor(white: 1.0, alpha: 0.0).cgColor
        return (panel, ManualPanelHost(contentView: hosting, in: panel))
    }
    ...
    private func layoutPanels(animated: Bool) {
        ...
        for bar in bars {
            ...
            let plan = bar.planLayout(contentWidth: bar.frozenContentWidth ?? measured, metrics: layoutMetrics)
            if plan.dockChanged {
                // 弹窗只在**它自己宿主屏**的任务条目标帧变化时关闭——别的屏宽度一变就关你的弹窗是错的。
                if folderPopupWantsOpen, popupHostScreenID == bar.id { closeFolderPopup() }
