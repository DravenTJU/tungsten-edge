...
final class ScreenBar {
    ...
    init(id: ScreenID, screen: NSScreen, dockPanel: NSPanel, capsulePanel: NSPanel) {
        self.id = id
        self.screen = screen
    ...
    func measureContentWidth(shadowPadding: CGFloat) -> CGFloat {
        guard let hosting = dockPanel.contentView else { return lastDesiredWidth }
        let measured = hosting.fittingSize.width - 2 * shadowPadding
    ...
    func teardown() {
        dockPanel.orderOut(nil)
        capsulePanel.orderOut(nil)
        dockPanel.contentView = nil
        capsulePanel.contentView = nil
    }
}
