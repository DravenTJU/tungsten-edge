...
struct DockStripView: View {
    ...
    private func makeProjection() -> StripProjection {
        ...
        // ⚠️ 喂给顺序层的永远是**全局并集**（projectedLive / liveOrderIDs / appKeys 都不过滤）。
        ...
        let messagingIDs = visibleMessaging.compactMap { entry -> String? in
    ...
    var body: some View {
        ...
        // 同样要加闸：消息区 id 现在是按屏过滤的，副屏消息区为空时这个条件恒真，
        // 不加闸会取消掉主屏发起的拖动。
