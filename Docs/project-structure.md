...
- `30-per-display-taskbar.md` — 每屏常驻任务条的决策记录（ADR）：owner 拍板的 12 条规则、为什么用 `CGDirectDisplayID`、为什么屏幕键进 `seatSignature` 而不是坐标、为什么刻意不注册 AX 移动通知。
- `31-per-display-taskbar-verification.md` — 同一改动的构建/运行/验收清单：当前进度、签名证书三步、13 项手动验收、`[screen]` 诊断日志速查、回退方式。
