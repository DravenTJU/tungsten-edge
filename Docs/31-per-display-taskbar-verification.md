...
> 配套文档：决策与理由见 [30-per-display-taskbar.md](30-per-display-taskbar.md)（ADR）；工程结构与签名证书详细步骤见 [project-structure.md](project-structure.md)。
>
...
一期（每屏一条常驻任务条 + 按屏过滤窗口）代码已完成，分三个检查点提交（提交号见
`git log origin/master..`，因为这条分支经历过一次重基）：

| 检查点 | 内容 |
|---|---|
| 1 | 窗口→显示器归属数据层（界面零变化） |
| 2 | 面板层重构为 `ScreenBar`，但仍然只建一条（安全阀） |
| 3 | 放开每屏一条常驻任务条 |

> **原检查点 0（修正 Quartz 坐标翻转基准）已作废**：上游在 v0.6/v0.7 期间独立修掉了
> 同一个 bug（`PanelCoordinator.quartzPrimaryScreenHeight`，注释里的理由与我们当初的分析
> 一致——`NSScreen.main` 跟着 key window 跑，可能是副屏）。移植时直接采用上游实现。

### 已验证 ✅

- **构建通过**，零 error。仅剩 4 条既有 warning，都在 `WindowZoomDemoController` 相关的 demo 代码里（`DOCK_ZOOM_DEMO` 门控的试验路径），非本次引入。
- **单元测试 746 个全过，0 失败**（上游自有 687 + 本次新增 59）。新增三个套件：
...
```bash
git log --oneline origin/master..   # 找到「检查点 2」那个提交
git checkout <检查点 2 的提交号>
```

检查点 1 是纯数据层（界面零变化），一般不需要回退。

---

## 四点五、已知缺口（移植到 v0.7.6 时划出范围）

**副屏上的最大化窗口不会避让副屏的任务条。**

上游在 v0.6/v0.7 期间把「最大化避让」整个重写了：`WindowZoomAvoidance*` → `WindowLiftAvoidance*`，
体量从几百行涨到一千五百多行，加入了权限丢失冻结、重抬、还原等一整套状态机。它把
`PanelCoordinator.windowLiftAvoidanceContext()` 的返回值当作**相等性快照**用——有 6 处
`host?.windowLiftAvoidanceContext() == context` 判断「任务条几何变了就放弃本次抬升」。

要做到按屏避让，就得把「被点窗口所在的屏」一路穿到这 6 处判断里，那是改上游逻辑本身，
不属于机械移植。移植时保留了上游实现（只认主屏那条栏），并在函数上方留了注释说明。
单独排期处理。

其余功能（每屏一条、按屏过滤、按屏全屏隐藏、抽屉/弹窗宿主屏、拖拽发起屏仲裁）均已完整移植。

## 五、二期待办

见 [30-per-display-taskbar.md](30-per-display-taskbar.md) 末节。要点：
