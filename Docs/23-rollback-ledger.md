...
**最近一次全表复检：2026-07-30（`121aebf` 图标淡化转正后，15 条逐条实跑 + 对照跑）**
...
**浅色这一轮（`f1efa7c` … `385f228` 共 11 个提交）互相层叠得很厚**，逐个单独回退基本都不成立；要整轮撤销就按提交倒序连着 revert，别挑单个。

| 条目 | 是否仍可干净回退 | 说明 |
| --- | --- | --- |
| `385f228` 退出档不再全灰 | ✅ 单点成立（最新提交，按上面那段默认它随时会废） | 2026-07-30 新增。owner 第二次微调：`light.iconDimNotRunning` 从 `1.00/0.60` 改成 `0.58/0.72`，两档挨近。回退只是把退出档变回全灰剪影，不影响别的 |
| `121aebf` 图标淡化转正 + 构建号 12 | ⚠️ **登记后隔一个提交就衰减**（`AGENTS.md` + `DockThemeTokens.swift` + `DockThemeTests.swift`） | 上一行的 `385f228` 改了同样这几个文件——**登记时那句「随时会废」当天就应验了，这是本表第三个同模式实例**。要单独回退它，先 revert `385f228`。2026-07-30 新增。浅色图标淡化从纯 `.opacity()` 改为三通道（去饱和 + 压暗），owner 实机验收后转正为默认、`DOCK_ICON_DIM` 开关随之删除。回退后浅色图标退回「灰蒙蒙」（深底图标褪成灰方块），深色不受影响（它的两格本来就是冻结的历史字面值）；构建号退回 11，与当前已装包撞号。**它同时碰了 `AGENTS.md` 和 `project.pbxproj` 两个高频文件**，是下面 `c6dfdb7` / `b7abf7e` 两条当场衰减的原因 |
...
| 目标 | 建议逆序 | 会保留什么 | 当前实证 |
| --- | --- | --- | --- |
| **撤销启动弹跳修复（动画作用域 + 启动会话）** | `git revert a645590` | 保留其余全部功能。回退后**三件事一起回来**：① 弹跳重新用 `repeatForever`，被 hover 事务（以及 `DockStripView` 的 `stripLayoutKeys`、`DrawerView` 的 `runningIDs`/`launchIDs`/`isPresented` 四处同类事务）顶掉后停死在 −6pt，即 owner 报的「鼠标移开就停在弹跳最高点」，图标尺寸同事务插值叠加成斜向乱跳；② 放行判定退回 `runningApplications.first(where: 同 bundleID)` + `activationPolicy != .regular`，启动早期 policy 未定型或同包名辅助进程命中即误放行（约 1 秒），兜底退回 8s 且两处计时器都不认会话，20 秒内二次启动会被旧计时器掐断；③ `RunningApplicationStore` 退回 `isTerminated` 判活、不再对未定型 policy 做有界重试，也不再处理「didTerminate 早于 POSIX 消失」——运行投影可能永久陈旧。删除 `Core/Support/LaunchGateDecision.swift` 及其单测、`--launch-trace` 模式与 `DOCK_LAUNCH_TRACE` 诊断 | **↩ 单点（登记当天）**：本提交即 `HEAD`，`git merge-tree` 模拟回退 exit 0。**它改了 `AGENTS.md` 且新增了源文件**，下一个碰这两处的提交就会压中它。无 schema 或 UserDefaults 数据边界。**验证状态**：全量单测 609/609；owner 于 2026-08-01 实机验收弹跳手感通过。**未覆盖**：`LauncherChip` 的计时器接线（创建 / 失效 / onAppear 重建）没有单测，纯判定 `LaunchGateDecision` 有 7 条；`RunningApplicationStore` 新增的两套重试全无单测，只有代码保证。**登记时已知两个未解问题**（不由本提交引入，但被 20s 兜底放大）：系统设置 `com.apple.systempreferences` 的窗口未被清点，图标一直是应用级占位 → 弹满 20s 且点击无法最小化；剪映 `com.lemon.lvpro` 点击打不开，需查是否被误判为「已在运行」而走了唤出分支——后者与本提交的 `RunningApplicationStore` policy 重试有嫌疑关联，回退前先读这一条 |
