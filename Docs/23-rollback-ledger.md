...
| 目标 | 建议逆序 | 会保留什么 | 当前实证 |
| --- | --- | --- | --- |
| **撤销启动期面板布局循环修复** | `git revert 328bc59` | 保留反馈计时器按需运行与 v0.7.1 全部产品功能；删除 `ManualPanelHost` 及其 7 项测试，主任务条、胶囊和 tooltip 恢复把 `NSHostingView` 直接装成 panel `contentView`，启动恢复「先显示、后异步收敛」，`$dockSize` 初值再次触发一次换档事务。**会重新暴露已复现的 AppKit Update Constraints 递归 / `NSGenericException` 启动崩溃** | **↩ 单点（merge-tree 实证）**：自身模拟回退 exit 0；无 schema 或 UserDefaults 数据边界。验证状态：全量 592/592、Debug 构建、Release 固定证书安装与签名校验；当前双屏四档各 25 次、总计 100 次冷启动全部保持 `type="UIElement"`，每次 4 秒 CG 采样均看到两个合法面板，未出现 `53×100` 窄窗，崩溃报告 6→6。抽屉开合和底边跨屏往返实测通过；tooltip 用真实 `NSHostingView` 可见 display-cycle 单测覆盖。未单独实操自动隐藏、全屏和最大化避让，相关代码未改且全量回归测试通过 |
