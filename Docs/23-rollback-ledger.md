...
**最近一次全表复检：2026-07-30（`c6dfdb7` 浅色定稿装机后，13 条逐条实跑）**

> 同日现场教训：`f1efa7c` 那次复检把自己登记成「✅ 单点成立」，**两个提交之后就不成立了**——`ed31ff2` 同时碰了 `DockStripView.swift`、`AGENTS.md` 和 `project.pbxproj`。这不是意外，是本表下方列的两个高频接触面加上功能文件本身，一次全踩。所以「刚登记的条目」和老条目一样不可信，读表永远先看日期和这一段。

**浅色这一轮（`f1efa7c` … `c6dfdb7` 共 7 个提交）互相层叠得很厚**，逐个单独回退基本都不成立；要整轮撤销就按提交倒序连着 revert，别挑单个。

| 条目 | 是否仍可干净回退 | 说明 |
| --- | --- | --- |
| `c6dfdb7` 构建号 10 → 11 | ✅ 单点成立 | 2026-07-30 新增。只改 `CURRENT_PROJECT_VERSION` 四处；回退后已装包与公开 v0.7.0 的版本行重新变得难以分辨，仅此代价 |
| `b7abf7e` 效果改 opt-in | ✅ 单点成立 | 2026-07-30 新增。回退后提饱和与厚度层恢复默认生效——也就是恢复到 owner 未认可的那个观感，**不要单独回退它** |
...
| `f1efa7c` 浅色模式视觉适配 | ⚠️ 不再是单点（5 个文件 + `project.pbxproj`） | 登记时是 ✅ 单点，两个提交后就被 `ed31ff2` 压出冲突，此后又被第二轮层层覆盖。**整轮浅色要撤销就倒序连 revert**：`c6dfdb7` → `b7abf7e` → `175cfa4` → `ed31ff2` → `f1efa7c` |
...
| **撤销整轮浅色（推荐用这条，不要挑单个提交）** | 倒序连着 revert：`git revert c6dfdb7 b7abf7e 175cfa4 ed31ff2 f1efa7c` | 回到 `f923324` 的浅色状态：白字白框黑阴影全部回来，深色不受影响。同时失去材质/提饱和/厚度层三个调参开关与玻璃探路装置 | **⇤ 整组**：这一轮 7 个提交层叠很厚（同一批视图文件被反复改），逐个单独 revert 基本都会冲突，2026-07-30 全表复检已逐条确认。整组倒序是唯一低摩擦路径 |
