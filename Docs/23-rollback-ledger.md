...
| 目标 | 建议逆序 | 会保留什么 | 当前实证 |
| --- | --- | --- | --- |
| **撤销任务条四档缩放** | `git revert 698a266` | 保留其余全部功能。菜单去掉「任务条大小 ▸」，面板几何回到写死的 52/92，`ShelfChip` / `PinnedFolderChip` 重新没有 `scale` 参数，胶囊和拖动载体不再观察 `AppSettingsStore`。**会一并丢掉一个顺手修的既有 bug**：条内标题宽度与 tooltip 截断阈值退回两处各写死 140pt（中档下行为相同，所以回退后看不出来，但那处耦合又埋回去了） | **↩ 单点（登记当天）**：本提交即 `HEAD`，此刻 revert 必然干净。**它改了 `AGENTS.md`**，下一个碰该文件的提交就会压中它。**UserDefaults 数据边界**：`com.tungsten.edge.dockSize` 会成为孤儿键（无害，不再被读）；回退时若用户停在非中档，任务条会**直接跳回中档**——这不是 bug，是该功能整体消失的必然结果，但要提前告诉用户。**验证状态**：全量 575/575；四档逐档装机实测面板高 84/92/100/108、胶囊同宽同高，小档与特大档截图确认图标/运行点/分隔线/胶囊九宫格无裁切；owner 于 2026-07-30 实机验收通过。**未覆盖**：从菜单**现场换档**那条事务路径（代次门控 / 关抽屉重测 / 无动画原子提交）只有单测与代码保证，实测走的是「改配置 + 重启」，没有真机点过菜单 |
