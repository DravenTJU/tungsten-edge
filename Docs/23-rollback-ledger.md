...
以下结论以 **2026-07-18 的 `8e2b851`**（`finder-folder-preview` 最新功能检查点；抽屉图标反馈为纯视觉层，无数据边界）为起点；其下的抽屉位置 / 保留拆分数据边界仍适用于 `5f5efa0`。旧提交即使产品上是独立能力，也可能因为后来改过同一文件而无法直接 `git revert`；“人工”表示要解决冲突并重新验证，不表示功能不能移除。
