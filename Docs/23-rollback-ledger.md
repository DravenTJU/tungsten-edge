...
**最近一次全表复检：2026-08-02（`a6a31f7` 系统 Dock 菜单组重做后，48 条逐条实跑 + 对照跑，零新增衰减；详见下方表格前的复检记录）**

**最近一次抽检：2026-08-02（`400bc9c` 悬停效果两档后，跑了当时还挂 ✅ 的 3 条）**——结果：**`0482a77` 被本轮压死**（对照跑 `--merge-base=0482a77 400bc9c^ 0482a77^` 退出码 0，确认提交前还干净；冲突落在 `DockStripView.swift` / `LauncherChip.swift` / `Docs/27`）、`97dacd5` 与 `8af961c` 仍干净。**这是本表第五个「刚登记就被下一个提交打掉」的实例**，而且 `0482a77` 自己那条登记时就写着「碰了 `AGENTS.md` 和 `Docs/27`，下一个改它们的提交就会让它衰减」——**预言当天应验**。⚠️ 这次同样没跑 ⚠️ 那些行。
