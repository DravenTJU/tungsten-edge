...
**浅色这一轮（`f1efa7c` … `121aebf` 共 9 个提交）互相层叠得很厚**，逐个单独回退基本都不成立；要整轮撤销就按提交倒序连着 revert，别挑单个。

| 条目 | 是否仍可干净回退 | 说明 |
| --- | --- | --- |
| `121aebf` 图标淡化转正 + 构建号 12 | ✅ 单点成立（**登记当天即为最新提交，按上面那段默认它随时会废**） | 2026-07-30 新增。浅色图标淡化从纯 `.opacity()` 改为三通道（去饱和 + 压暗），owner 实机验收后转正为默认、`DOCK_ICON_DIM` 开关随之删除。回退后浅色图标退回「灰蒙蒙」（深底图标褪成灰方块），深色不受影响（它的两格本来就是冻结的历史字面值）；构建号退回 11，与当前已装包撞号。**它同时碰了 `AGENTS.md` 和 `project.pbxproj` 两个高频文件**，是下面 `c6dfdb7` / `b7abf7e` 两条当场衰减的原因 |
...
| **撤销图标三通道淡化（只回图标，不动整轮浅色）** | `git revert 121aebf` | 保留整轮浅色的其余全部成果。失去的是：浅色下变灰的图标退回纯 `.opacity()`（隐藏 0.45 / 退出 0.35），也就是 owner 报的「灰蒙蒙」重现，深底图标重新褪成灰方块；两档重新只靠那点透明度差区分。**深色观感不受影响**——它的两格本来就是冻结的历史字面值。构建号退回 11，与当前已装包撞号 | **↩ 单点**：2026-07-30 `git merge-tree` 实测干净（登记当天即最新提交，随时会被下一个碰 `AGENTS.md` 或 `project.pbxproj` 的提交压废）。全量测试 SUCCEEDED（556 条）。**验收依据是实机**：装 `0.7.0 (12)` 后不带任何环境变量正常启动，owner 认可；代码侧另有证明——`legacyHidden`/`legacyNotRunning` 现在只被深色列引用，浅色没有任何路径能回落到旧值 |
| **撤销整轮浅色（推荐用这条，不要挑单个提交）** | 倒序连着 revert：`git revert 121aebf c6dfdb7 b7abf7e 175cfa4 ed31ff2 f1efa7c` | 回到 `f923324` 的浅色状态：白字白框黑阴影全部回来，深色不受影响。同时失去材质/提饱和/厚度层三个调参开关、玻璃探路装置，以及图标淡化的三通道处理（图标退回纯 `.opacity()`，浅色下重新「灰蒙蒙」） | **⇤ 整组**：这一轮 9 个提交层叠很厚（同一批视图文件被反复改），逐个单独 revert 基本都会冲突，2026-07-30 全表复检已逐条确认。整组倒序是唯一低摩擦路径 |
