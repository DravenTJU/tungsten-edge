...
以下结论以 **2026-07-16 的 `489f5d5`**（`finder-folder-preview` 最新检查点）为起点。旧提交即使产品上是独立能力，也可能因为后来改过同一文件而无法直接 `git revert`；“人工”表示要解决冲突并重新验证，不表示功能不能移除。
...
| **恢复避让试验到主线**（找回，不是撤销） | `git merge experiment/avoid-scale-hover` 或挑单个提交 `git cherry-pick 229a6a5 6ca3992 4b58365` | 主线当前全部功能 + 避让试验代码 | **↩ 单点**：`experiment/avoid-scale-hover` 完整封存了这条思路；也可以直接 `git revert 63f7ee8 299748a 9ff1aec`（撤销这三次撤销）达到同样效果，未单独验证 |
