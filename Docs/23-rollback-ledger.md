...
**最近一次抽检：2026-08-02（`0482a77` 拿掉图标淡化后，只跑了当时还挂 ✅ 的 4 条）**——结果：`385f228` 被本轮压死（对照跑确认提交前还干净）、`203b69c` 早就被 `edf9b64` 压死（**不是本轮，是上次全表复检漏掉的**）、`97dacd5` 与 `8af961c` 仍干净。**⚠️ 这次没跑 ⚠️ 那些行**，它们的说明可能同样过时。
