...
**最近一次全表复检：2026-07-30（`ed31ff2` 玻璃探路后；同日更早对 `f1efa7c` 做过一次 11 条全表实跑）**
...
| 条目 | 是否仍可干净回退 | 说明 |
| --- | --- | --- |
| `ed31ff2` 原生玻璃探路装置 | ✅ 单点成立 | 2026-07-30 新增，见下表条目。默认关的探路装置，回退它不改变任何既有行为 |
| `f1efa7c` 浅色模式视觉适配 | ⚠️ **两个提交之内就衰减**（`AGENTS.md` + `DockStripView.swift` + `project.pbxproj`） | 登记时是 ✅ 单点，`ed31ff2` 就把它压出冲突——玻璃探路改了 `DockStripView` 的底板那一行、往 AGENTS.md 同一节追加了结论、又新增一个源文件，**三个高频公共接触面一次踩全**。要回退先 `git revert ed31ff2` 再 `git revert f1efa7c`，两条按序就干净 |
