...
| 条目 | 是否仍可干净回退 | 说明 |
| --- | --- | --- |
| `62e212e` 最大化避让做成菜单开关，默认关 | ✅ 单点成立（最新提交，按上面那段默认它随时会废；本次登记提交自身碰 `Docs/23` / `Docs/26` / `Docs/28`，本条都没带，暂时不自伤） | 2026-08-03 新增，登记当时 `git merge-tree` 退出码 0。**这是一条对用户可见的行为反转**：回退后避让重新变成恒开、不可关，菜单里「最大化窗口避开任务条」整项消失。用户已存的 `com.tungsten.edge.windowLiftEnabled` 键会变成孤儿键（不删，属于用户数据）——注意**回退后勾没勾过都一样恒开**，也就是主动关掉过的人会被重新打开，这是回退本条唯一会让用户"意外"的地方。碰了 `AppSettingsStore.swift` / `StatusMenuController.swift` / `AppDelegate.swift` / `WindowLiftAvoidanceController.swift` / `AppSettingsStoreTests.swift` + `AGENTS.md` + `Docs/27` 两个高频接触面——**下一个改它们中任意一个的提交就会让它衰减**（它自己正是这么打掉 `7b61767` 的）。回退不涉及避让本身的任何逻辑：`start()` 里只多一句 `guard isEnabledBySetting`，控制器其余部分一字未动。**验证状态：630/630 单测全过（新增 1 条）、Debug 构建 + AX 探针实测四条路径（默认关不抬 949pt / 勾上后 949→889 抬起 / 抬着的状态取消勾选 2 秒内 889→949 还原 / 再勾回来重新抬起）、跨重启保持、`DOCK_WINDOW_LIFT=0` 时勾着也不抬；owner 实机验收待完成** |
