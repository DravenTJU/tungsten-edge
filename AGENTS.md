...
- **Drop-hover highlight must not rely on terminal callbacks or `folderPaths` changes to clear.** SwiftUI file drop is unreliable at the end: (a) a drag can end after the last `dropUpdated` with **no** `performDrop`/`dropExited` at all — the `.pin` highlight strands forever (the "白边" bug); (b) after a successful `performDrop`, the system emits **one stray `dropUpdated` ~330ms later** that re-lights it (post-drop 回闪). Adding more resets inside `performDrop` cannot fix either (the stuck cases are exactly when `performDrop` doesn't run / runs before the stray), and `PinnedFolderStore.insert` no-ops on a same-position re-drop so `.onChange(of: folderPaths)` is not a reliable clear either. Fix = two parts, both scoped to `externalDropTarget` only (never touch the shared drawer-unstash `stripHighlighted` path): **(1) an entry-gated highlight** — only `dropEntered` may light it (`externalDropHoverActive`); a lone post-drop `dropUpdated` (session already ended) is ignored → kills 回闪. **(2) a `.common` run-loop watchdog** — each hover update re-arms a ~0.35s one-shot `Timer` (`.common` mode is load-bearing; hover updates come every ~50ms so it never false-fires mid-hover, including a stationary hold); if updates stop with no terminal callback it clears the stranded highlight. A `generation` counter guards against a queued stale timer clearing a fresh session. Do not revert to provider-`ObjectIdentifier` session identity — provider identity is **not** stable within one SwiftUI drop session (empirically varies across callbacks).
