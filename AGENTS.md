...
- AX title reads preserve three states: non-empty value, known empty/unsupported, and unread. An unread title may inherit the previous title only while the same `pid + cgWindowID` continues in its existing seat; a known empty title clears it, and tab replacement, tear-out replacement, and new seats never inherit another window's title. Do not collapse field-level unread back into empty: messaging-main absorption depends on the last known title remaining stable through transient AX timeouts.
- The 0.5s frontmost-window poll keeps its cadence, but its full `AXWindows` read must use the timed background read pipeline. Do not put `reconcileSeats`' untimed AX read back on the main actor: a responsive-looking app can still block it for 200–400ms every poll and visibly freeze chip animations. CG on-screen changes must still force the same convergence rebuild when the background result lands.
- **A successful AX read is not proof that its window list is complete.** An empty (or short) `AXWindows` result must never release a seat: a live, non-tombstoned CG id retains its seat indefinitely, and the old `absentSince` / `closedReapGrace` absence-reaping path is deleted (`.absentBeyondGrace` survives only as a historical log-schema value and must not be emitted). Release comes from process death, CG-list disappearance / destroy tombstone, or phantom healing — never from AX absence alone.
...
- **The "app name" side of `titleMatchesAppName` must come from the monotonic `AppNameRegistry` (`Core/Support/AppDisplayNameCache.swift`) — never a per-frame LaunchServices lookup.** `NSRunningApplication.runningApplications(withBundleIdentifier:)` transiently returns empty for a live process; this is the **second exit of the same pit** as the `ProcessLiveness` rule above, which only sealed the liveness question. Not theoretical (measured 2026-08-11): 飞书's and 微信's bundle-derived names resolve to `{feishu, lark}` / `{wechat}` on a zh-Hans system — their Chinese window titles are **not in the set at all**, so both match *only* through that lookup and fail together on one hiccup, while QQ's `{qq}` hits the cached path and never flaps. Each failure renders the app as a chip in the messaging zone **and** a window chip in the live zone, widening the row and shoving everything right, and the failed state latches until an unrelated re-render (measured 2s–79min). The registry's name set is monotonic (bundle-derived ∪ every `localizedName` ever observed) and **must keep retrying until the first successful observation** — caching a partial set on a cold-start hiccup would turn a transient fault into a permanent one, strictly worse than the original bug. Regression lock: `AppNameRegistryTests`. Diagnosing this cost three wrong guesses; the reasoning that worked is in `Docs/28-process-pitfalls.md`.
...
- Chip hover presentation has one animated scalar (`ChipHoverProgress.animatableData`). Pill height/shift, icon size, subtitle width/opacity, fill and rim all derive from the same `ChipHoverVisual`; icon-only, titled and launcher chips share it. Do not add per-property hover `.animation` modifiers. `DOCK_CHIP_ANIM_TRACE=1` is the default-off presentation trace.
- **Press feedback is driven by mouse-DOWN, and all of it lives in `ChipPressFeedback`** (owner reported 「按下去有粘滞感」 2026-08-11). It used to hang off `.onTapGesture`, which SwiftUI fires on mouse-**up** — so the whole 0.93 dip played *after* the click was already over, which is what the stickiness was. The driver is now `simultaneousGesture(DragGesture(minimumDistance: 0))`; the codebase precedent for composing that with `.onTapGesture` + `.nativeContextMenu` is `chipWithReorder`'s existing 8pt reorder gesture. Load-bearing: (a) the release keeps the old **90ms minimum hold** or a fast click shows nothing (`ChipPressDecision.holdAfterRelease`, unit-tested, and it must never return a negative delay); (b) a **watchdog** (`maximumHold` 1s) is mandatory because SwiftUI cancels the gesture when a reorder moves the chip, so `onEnded` may never arrive and the chip would stay at 0.93 forever; (c) the two edges use **different** curves — down `easeOut 0.08`, release the historical `spring(0.22, 0.5)`, which stays byte-identical because it is the signed-off look; (d) scale and gesture are **separate** modifiers (`chipPressScale` / `chipPressGesture`) because the drawer capsule scales only its inner grid while the gesture must sit on the whole capsule. Kill switch `DOCK_CHIP_PRESS_DOWN=0` restores the old mouse-up pulse. Four surfaces share it — window chips, `LauncherChip` (kept / messaging-without-main-window / drawer icons, which had **no** press feedback at all before this), and `DrawerCapsuleButton`; `LauncherChip` passes `isEnabled: !isLaunching` because taps are already a no-op during a launch session. `PinnedFolderChip` / `ShelfChip` deliberately stay out (their popup is the feedback).
- **User actions run on `AppRuntime.actionQueue` (`.userInitiated`, concurrent) — never `Task.detached`.** `AppTracker`'s background AX reads use the Swift cooperative pool, including a **serial** `for` loop in the 5s `scanNonAdmittedApps` that can hold a pool thread for N×100ms; blocking AX calls do not belong there and a click must not queue behind an inventory read.
- **Window-handle capture is three tiers: `AXElementCache` → cgWindowID fast probe (100ms) → full title/frame chain (500ms).** The cache is a **side table**, keyed `pid + cgWindowID`, written by `AppTracker` from the inventory reads it already performs. Do **not** move it into `DockSnapshot` / `WindowRecord`: those are equality-gated for rendering, and a per-round-changing `AXUIElement` there would re-render the whole strip twice a second — the exact defect `4120119` fixed. Invalidation mirrors seat release — window destroy, process removal, and CG **full-list** intersection each reconcile — and must **never** be "absent from AX this round": Safari-style windows leave `AXWindows` when minimized, which is precisely when the cache earns its keep. A cached element is verified by one `_AXUIElementGetWindow` under a 50ms budget with a **three-state** result: match → use, mismatch → drop the entry (cgWindowID reuse), unread → fall through but **keep** the entry (a transient timeout must not be frozen into permanent failure). `justUnhid` disables the first two tiers, as before. Kill switch `DOCK_AX_ELEMENT_CACHE=0`.
- `activate()`'s `knownMinimized` is an **affirmative-only** fast path: a snapshot saying `.minimized` skips the live `kAXMinimizedAttribute` read, but a snapshot saying *not* minimized must still read, or one stale snapshot leaves the window unrestorable. `DOCK_CLICK_TRACE=1` writes the per-click stage timeline (`execStart` / `earlyFocus` / `handle` + which tier / total) to `~/Library/Logs/com.caye.macosdockcc.v2/click-latency.jsonl`; it is the ruler for any future claim about click latency — do not judge this path from constants again.
...
- Window actions try the known cgWindowID handle under one 100ms total budget, then use the existing full title/frame chain (the `AXElementCache` tier above now runs before both); `DOCK_FAST_WINDOW_HANDLE=0` disables the fast attempt. `DOCK_CHIP_PROBE` defaults off and only `=1` may add planner/minimize diagnostic reads. If a `.minimizeWindow` handle cannot be captured, return failure and let optimistic UI roll back; never hide the whole app unless temporary compatibility switch `DOCK_MINIMIZE_APP_FALLBACK=1` is set.
