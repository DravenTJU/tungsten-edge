...
- Kept state is an exit-retention flag, not placement. It may coexist with drawer placement, and `AppMembershipController.setKept(_:enabled:)` mutates only `KeptAppStore`. Kept remains mutually exclusive with messaging identity; messaging entries never expose the keep checkbox.
...
- `LauncherChip` menu running-state follows the passed-in `isRunning` (the displayed zone), never an independent `NSWorkspace` process query. A launch-zone icon whose process is still alive (window-closed / background) must not surface 显示/隐藏/退出. The pure decision is `LauncherMenuPlan.itemKinds` (unit-tested); `buildLauncherMenu` only renders it and queries `NSWorkspace` solely to obtain the app object for an action it already decided to show. Membership items are injected via `membershipItems` and may be multiple (for example, keep plus mark-as-messaging). A messaging-zone or messaging drawer icon shows only 取消标记消息应用 — never 在程序坞中保留; both messaging main-window cards and pop-out cards keep 取消标记消息应用.
