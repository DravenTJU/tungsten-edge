...
- The two 「自动隐藏」 checkbox rows (one per slider group; the edge group also has global hot key ⌥⌘E) toggle their group's delay between `neverHideDelay` and persisted `lastEnabled*Delay` — they reuse the slider pipeline, never a separate visibility state. Remembered values must never be `-1`; seeding/sanitizing lives in `AppSettingsStore` (unit-tested, per-group fallbacks 0.1/1.0). The combo must not contain Control+Option (VoiceOver's VO modifier space) nor be Option-only/Option+Shift-only (macOS 15 bug FB15168205). ⌥⌘E deliberately mirrors the system's ⌥⌘D and knowingly shadows Safari Develop → Empty Caches and Finder File → Eject All / 推出所有磁盘 while running; the owner accepted both low-frequency conflicts on 2026-07-17 in exchange for the memorable default.
- The native-Dock checkbox row derives check state and click direction from the LIVE system state (`NativeDockPreferencesService.currentAutohideState`; `CFPreferencesAppSynchronize` before every read — external ⌥⌘D / System Settings changes never enter the process cache otherwise), falling back to store state only when unreadable. `menuWillOpen` reconciles the store to system truth (`AutoHideToggleMenuModel.reconciledStoreDelay`) so slider + checkbox agree, and must never apply back (no killall Dock on menu open). There is deliberately no blind store-flip toggle for the native group — a blind flip goes the wrong way once external changes desync the store.
- Global hot key rules (`GlobalHotKeyMonitor`, Carbon `RegisterEventHotKey`; exclusivity passes through the backend protocol so tests assert it): lifecycle is main-thread only; the callback may only toggle settings — it must not export debug snapshots or synchronously read cross-app AX/CG inventories. History (facts only, root cause unverifiable): `417d93d` added a SwiftUI-local ⌘⇧D + `SIGUSR2` debug-snapshot trigger, `ef50008` deleted it in the big pipeline rewrite without mentioning it, `1ed0e66` retroactively documented "间歇主线程卡死已回退". Menu `keyEquivalent` hints: ⌥⌘E shows only when Carbon registration succeeded (failure logs once, no dialog, no in-session retry; a deliberate `stop()` re-arms `start()` for a real re-registration); ⌥⌘D always shows (system-owned, no registration involved). Live-verified boundary (owner accepted 2026-07-17): with the menu closed, ⌥⌘E runs through Carbon and ⌥⌘D through macOS; while the menu is open, ⌥⌘E is unavailable under the current exclusive-Carbon/menu-tracking interaction, so use the Tungsten Edge menu item directly, whereas macOS's ⌥⌘D still executes and the native-Dock menu action must skip its exact keyDown. Native-Dock matching uses physical `keyCode`, never character (non-US layouts diverge); mouse click, Return selection, and accessibility activation must still execute both menu actions.
