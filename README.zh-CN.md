...
- **菜单栏控制**：状态栏菜单里可以设置登录时启动、系统 Dock 显隐与唤醒时间、钨极自己的唤醒时间、要不要显示中转站，以及任务条大小。
...
- **隐藏/显示系统 Dock**：等价于 macOS 自带的 `⌥⌘D`，只切换原生 Dock 的自动隐藏，不改动你自己设的唤醒延迟。菜单里直接显示 `⌥⌘D` 作为快捷键提示（这条快捷键由 macOS 持有，恒生效）。下面的紧凑滑杆调整原生 Dock 的唤醒时间：`常驻`、`0.1s`–`3.0s` 或 `不唤醒`——拖到 `不唤醒` 就是彻底收起，鼠标碰到底边也不会弹出。滑杆松手才写入（每次写入都会重启 Dock）。
- **打开系统 Dock 设置…**：打开系统设置里的 Dock 配置页（Ventura 及以上为「桌面与程序坞」，macOS 12 为「程序坞与菜单栏」）。它只负责跳转，不写偏好、不重启 Dock。
- **隐藏/显示 Tungsten Edge 钨极**：在常驻和上一次的自动隐藏唤醒时间之间切换。下面的紧凑滑杆调整唤醒时间：`常驻`、`0.1s`–`3.0s` 或 `不唤醒`。状态菜单关闭时，全局快捷键 `⌥⇧⌘D` 执行同一切换；注册失败时菜单不显示快捷键提示，但命令仍可点击。它是在系统 Dock 的 `⌥⌘D` 上增加 Shift，同时把原 `⌥⌘E` 释放回 Safari「清空缓存」和 Finder「推出所有磁盘」。
- **显示中转站**：勾选项，控制中转站图标显不显示。取消勾选只是把图标藏起来，已暂存的文件引用照旧保留，勾回来还在。注意：中转站关掉**而且**一个固定文件夹都没有时，整个文件夹区会消失，任务条上就没有拖文件进来的落点、也没有「添加文件夹…」入口了——勾回中转站即可恢复。
- **任务条大小 ▸**：小 / 中 / 大 / 特大四档，任务条和右侧胶囊一起缩放——图标、文字、间距、圆角和条高都跟着走。默认是「中」，与旧版本逐像素一致。换档瞬时生效、不做过渡动画；打开着的抽屉会先关掉，好按新尺寸重新测量。抽屉内部内容和文件夹 / 中转弹窗维持原尺寸不缩放。
- **检查更新…**：手动检查 GitHub 上的最新正式版本；发现新版本后会打开对应 Release 页面，由你下载安装。

通过菜单直接修改系统 Dock 显隐需要非沙箱版本，因为沙箱应用不能写 Dock 偏好或重启 Dock；打开系统设置页本身不受此限制。
