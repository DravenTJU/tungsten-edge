...
- **`系统 Dock`**：调整系统原生 Dock 的唤醒等待时间。拖动时会先保存到钨极偏好；松手且数值确实变化后，会直接应用到系统 Dock，并重启系统 Dock 让设置生效。
- **自动隐藏（系统 Dock 组）**：滑杆正下方的勾选项，让系统 Dock 在常驻显示和上一次的唤醒时间之间切换。鼠标点击后，钨极会按切换后的滑杆值写入系统设置，并重启系统 Dock 让它生效。勾选项右侧的 `⌥⌘D` 是 macOS 自带快捷键，并不是菜单点击所走的执行路径。勾选状态每次打开菜单时都从系统实际设置读取——就算你在别处按过 `⌥⌘D`，这里也不会显示错。
- **`Tungsten Edge 钨极`**：调整鼠标停在底边多久后唤醒钨极。`常驻` 表示一直显示，有限值范围是 `0.1s` 到 `3.0s`，`不唤醒` 表示仍会自动隐藏，但底边不唤醒。
- **自动隐藏（钨极组）**：同样的勾选项，开关钨极任务条的自动隐藏（去掉勾 = 常驻显示，打上勾 = 回到你上一次的唤醒时间）；全局快捷键 `⌥⌘E` 在任何应用里都能按——和系统 Dock 的 `⌥⌘D` 同一对修饰键，`E` 就在 `D` 旁边。这也是 `不唤醒` 模式下把已隐藏任务条唤回来的最快方式。若钨极未能完成独占全局热键注册，菜单不会显示快捷键提示，但勾选项仍可点击。注意：钨极运行期间会独占 `⌥⌘E`，因此会覆盖 Safari「开发 → 清空缓存」和 Finder「文件 → 推出所有磁盘」的快捷键；这些功能普通用户很少使用，项目所有者已知情并接受这两个冲突，以换取更容易记住的默认快捷键。
