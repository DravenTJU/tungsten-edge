...
- **隐藏/显示系统 Dock**：隐藏时同时开启自动隐藏并把底边唤醒延迟设为“不唤醒”，因此原生 Dock 会彻底收起、鼠标碰到底边也不会弹出；显示时关闭自动隐藏，并恢复隐藏前原有的唤醒延迟。macOS 自带的 `⌥⌘D` 只能切换普通自动隐藏（碰底边仍会弹出），效果与本命令不同，因此菜单不把它显示为本命令的快捷键。
...
使用钨极菜单的「隐藏系统 Dock」后，鼠标停在底边不会再唤醒原生 Dock；选择「显示系统 Dock」会恢复隐藏前的唤醒延迟。
